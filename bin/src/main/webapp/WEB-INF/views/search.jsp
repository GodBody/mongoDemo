...
<div class="container">
                    ...
                    <tr>
                        <c:if test="${champion.winCount + champion.lossCount >= 400}">
                            ...
                            <td>${champion.winCount + champion.lossCount} games
                                ( ${champion.winCount}W ${champion.lossCount}L )
                            </td>
                            <td><fmt:formatNumber
                    ...
                    <tr>
                        <td>
                            ...
                            <c:set var="name" value="${fn:split(list.championName, '.')}"/>
                                ${name[0]}</td>
                        <td>${list.gameCount} games ( ${list.winCount}W ${list.looseCount}L )</td>
                        <c:if test="${list.winRate >= 60}">
                            <td><strong style="color: #c6443e"><fmt:formatNumber value="${list.winRate}"
    ...
    <div>
        ...
        <c:forEach items="${matchList}" var="match">
            ...
            Start Time :<fmt:formatDate value="${dateValue}" pattern="yyyy년 MM월 dd일 HH시 mm분 ss초"/>
            <jsp:setProperty name="dateValue" property="time" value="${match.gameDuration * 1000}"/>
            ( Elapsed time :<fmt:formatDate value="${dateValue}" pattern="mm분 ss초"/> )
            <br>
            ...
            <table class="table table-condensed">
                ...
                <c:forEach items="${match.participants}" var="participant" begin="0" end="4">
                    ...
                    <c:set var="htier" value="${participant.highestAchievedSeasonTier}"/>
                    <c:set var="htierL" value="${fn:toLowerCase(htier)}"/>
                    <td><img src="//opgg-static.akamaized.net/images/medals/<c:url value="${htierL}"/>_1.png"
                             width="40px"
                             height="40px" alt="unranked"></td>
                    ...
                    <td>${participant.stats.kills} / <strong
                        ...
                        ( <c:if test="${participant.stats.kdaRatio <3.00}">
                            <strong><fmt:formatNumber value="${participant.stats.kdaRatio }" pattern="0.00"/></strong>
                        </c:if>
                        <c:if test="${participant.stats.kdaRatio >= 3.00 && participant.stats.kdaRatio <4.00}">
                            <strong style="color: #2daf7f"><fmt:formatNumber value="${participant.stats.kdaRatio }"
                                                                             pattern="0.00"/></strong>
                        </c:if>
                        <c:if test="${participant.stats.kdaRatio >= 4.00 && participant.stats.kdaRatio <5.00}">
                            <strong style="color: #1f8ecd"><fmt:formatNumber value="${participant.stats.kdaRatio }"
                                                                             pattern="0.00"/></strong>
                        </c:if>
                        <c:if test="${participant.stats.kdaRatio >= 5.00}">
                            <strong style="color: #e19205"><fmt:formatNumber value="${participant.stats.kdaRatio }"
                                                                             pattern="0.00"/></strong>
                        </c:if> )
