...
public class HomeController {
	...
	public static <T> HttpEntity<T> setHeaders() {
		...
		headers.set("X-Riot-Token", "RGAPI-95e4c911-a5e0-4f91-84d7-97224562edcb");
	...
	public String search(@RequestParam(value = "summonerName") String name, Model model) throws Exception {
		...
		SummonerDTO summoner = mongo.findOne(Query.query(Criteria.where("_id").is(summonerId)), SummonerDTO.class,
				"Summoner");
		...
		for (int i = 0; i < summoner.getMatches().size(); i++) {
			...
			for (int j = 0; j < match.getParticipants().size(); j++) {
				...
				ChampionDTO temp = mongo.findOne(Query.query(Criteria.where("_id").is(id)), ChampionDTO.class,
						"Champion");
				...
				if (!match.isComplete()) {
					if (p.getStats().isWin()) {
						mongo.updateFirst(Query.query(Criteria.where("_id").is(p.getChampionId())),
								new Update().inc("winCount", 1), "Champion");
					} else {
						mongo.updateFirst(Query.query(Criteria.where("_id").is(p.getChampionId())),
								new Update().inc("lossCount", 1), "Champion");
					}
				}
				SummonerSpellDTO spell1 = mongo.findOne(Query.query(Criteria.where("_id").is(id1)),
						SummonerSpellDTO.class, "SummonerSpell");
				SummonerSpellDTO spell2 = mongo.findOne(Query.query(Criteria.where("_id").is(id2)),
						SummonerSpellDTO.class, "SummonerSpell");
			...
			mongo.updateFirst(Query.query(Criteria.where("_id").is(gameId)), new Update().set("complete", true),
					"Match");
	...
	public void insert(String name) throws HttpClientErrorException {
		...
		try {
			...
			HttpEntity<SummonerDTO> requestEn = setHeaders();
			ResponseEntity<SummonerDTO> responseEn = restTemplate.exchange(searchByName + name, HttpMethod.GET,
					requestEn, SummonerDTO.class);

			headers = responseEn.getHeaders();

			SummonerDTO summonerDTO = responseEn.getBody();

			SummonerDTO summonerDB = mongo.findOne(Query.query(Criteria.where("_id").is(summonerDTO.getId())),
					SummonerDTO.class, "Summoner");
			...
			if (!check) {
				...
				ResponseEntity<Set> responseEn2 = restTemplate.exchange(tierById + String.valueOf(summonerDTO.getId()),
						HttpMethod.GET, requestEn2, Set.class);
				...
				ResponseEntity<MatchListDTO> responseEn3 = restTemplate.exchange(
						recent + String.valueOf(summonerDTO.getAccountId()) + "?endIndex=20", HttpMethod.GET,
						requestEn3, MatchListDTO.class);
				...
				for (int i = 0; i < matches.size(); i++) {
					...
					ResponseEntity<MatchDTO> response = restTemplate.exchange(matchById + matches.get(i).getGameId(),
			...
			// 갱신 될 정보가 있다면?
				...
				SummonerDTO temp = mongo.findOne(Query.query(Criteria.where("_id").is(summonerDTO.getId())),
						SummonerDTO.class, "Summoner");
				...
				for (int i = matches.size() - 1; i >= 0; i--) {
					if (matches.get(i).getTimestamp() > matchesDB.get(0).getTimestamp()) {
						...
						System.out.println("NEW gameId : " + matches.get(i).getGameId() + "/ championId : "
								+ matches.get(i).getChampion());
					}
				}
	...
	public void updateData() {
		...
		ResponseEntity<ChampionListDTO> responseC = restTemplate.exchange(championList, HttpMethod.GET, requestC,
		...
		ResponseEntity<ItemListDTO> responseI = restTemplate.exchange(itemList, HttpMethod.GET, requestI,
				ItemListDTO.class);
		...
		ResponseEntity<SummonerSpellListDTO> responseS = restTemplate.exchange(spellList, HttpMethod.GET, requestC,
